...
describe("merge", () => {
  ...
  it("merges fields from same key across patches", () => {
    expect(
      merge({ "e1/Position": { x: 10 } }, { "e1/Position": { y: 20 } }),
    ).toEqual({
  ...
  it("later values override earlier ones for same field", () => {
    expect(
      merge({ "e1/Position": { x: 10 } }, { "e1/Position": { x: 50 } }),
    ).toEqual({
      "e1/Position": { x: 50 },
    });
  });
  ...
  it("handles _exists flag merge", () => {
    expect(
      merge(
        { "e1/Position": { _exists: true, x: 0, y: 0 } },
        { "e1/Position": { x: 10 } },
      ),
    ).toEqual({
...
describe("subtract", () => {
  ...
  it("handles nested value comparisons (arrays)", () => {
    expect(
      subtract(
        { "e1/Shape": { pos: [10, 20], size: [50, 50] } },
        { "e1/Shape": { pos: [10, 20], size: [100, 100] } },
      ),
    ).toEqual({
      "e1/Shape": { size: [50, 50] },
    });
