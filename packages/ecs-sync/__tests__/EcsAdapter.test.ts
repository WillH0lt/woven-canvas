...
import { SINGLETON_STABLE_ID, type Mutation } from "../src/types";
...
function setup() {
  const world = new World(
    [Synced, Position, Velocity, Linked, Unsyncable, Camera, UnsyncedSingleton],
    { maxEntities: 1000, maxEvents: 4096 },
  );
...
describe("EcsAdapter", () => {
  ...
  describe("push", () => {
    ...
    it("applies component data when adding", () => {
      ...
      adapter.push([
        wsMutation({ "uuid-1/Position": { x: 100 } }),
      ]);
    ...
    it("re-uses an existing entity for the same stableId", () => {
      ...
      adapter.push([
        wsMutation({ "uuid-1/Position": { _exists: true, x: 10, y: 20 } }),
      ]);
      adapter.push([
        wsMutation({ "uuid-1/Velocity": { _exists: true, vx: 1, vy: 2 } }),
      ]);
  ...
  describe("lazy initialization", () => {
    ...
    it("initializes component maps on first push via pull dependency", () => {
      ...
      adapter.push([
        wsMutation({ "uuid-2/Position": { _exists: true, x: 30, y: 40 } }),
      ]);
