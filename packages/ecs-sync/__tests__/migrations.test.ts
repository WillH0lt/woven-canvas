...
describe("validateMigrations", () => {
  ...
  it("rejects superseding a migration that does not exist", () => {
    expect(() =>
      validateMigrations([
        { name: "a", supersedes: "nope", upgrade: (d) => d },
      ]),
...
describe("migrateComponentData", () => {
  ...
  it("throws on unknown version", () => {
    ...
    expect(() => migrateComponentData({}, "nope", migrations)).toThrow(
      'Unknown migration version "nope"',
    );
  });
