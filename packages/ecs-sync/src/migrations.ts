  ...
  for (const m of migrations) {
    ...
    if (m.supersedes != null) {
      if (!seen.has(m.supersedes)) {
        throw new Error(
          `Migration "${m.name}" supersedes "${m.supersedes}", which ${
            migrations.some((o) => o.name === m.supersedes)
              ? "must come before it"
              : "does not exist"
          }`,
        );
      ...
      if (supersededBy.has(m.supersedes)) {
        throw new Error(
          `Migration "${m.supersedes}" is already superseded by "${supersededBy.get(m.supersedes)}"`,
        );
      }
...
export function migratePatch(
  ...
  for (const [key, value] of Object.entries(patch)) {
    ...
    const migrationResult = migrateComponentData(
      componentData,
      _version ?? null,
      def.migrations,
    );
