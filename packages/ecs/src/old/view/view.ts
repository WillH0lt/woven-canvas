...
export class View {
  ...
  constructor(public maxView = new DataView(new SharedArrayBuffer(8192))) {}

  create<T>(
    schema: ViewSchema<T>,
    constructor?: T extends object ? Constructor<T> : never
  ): ViewConstructor<T> {
    const { getSchemaId, getSchemaOrdering } = this.constructor as typeof View;
    ...
    for (let i = schemas.length - 1; i >= 0; i--) {
      ...
      const View =
        objectSchema.btype === "map"
          ...
          : this.Views.get("object")!.initialize(
              objectSchema,
              this,
              objectCtor
            );
  ...
  getArrayView<T>(
    ...
    return this.Views.get(isTypedArray ? "typedarray" : "array")!.initialize(
      schema,
      this,
      SchemaView,
      itemLength
    ) as ViewConstructor<Array<T>>;
  }
  ...
  static getDefaultData<T extends unknown>(
    ...
    const buffer = new SharedArrayBuffer(viewLength);
  ...
  static getDefaultConstructor<T>(
    ...
    return new Function("return {" + content.join(",") + "}") as Constructor<T>;
  }
    ...
    while (processing.length) {
      ...
      for (const property of properties) {
        let field: ViewSchema<unknown> = (
          object.properties as Record<string, ViewSchema<unknown>>
        )[property];
        if (field.type === "array") {
