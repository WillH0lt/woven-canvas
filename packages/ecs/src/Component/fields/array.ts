...
export const ArrayField: Field = {
  ...
  growStorage(
    ...
    const newBuffer = new BufferConstructor(newCapacity * alignedBytesPerEntry);
    const newView = new ArrayBufferView(
      newBuffer,
      newCapacity,
      alignedBytesPerEntry,
      config.elementDef,
    ...
    const oldView = oldArray as ArrayBufferView;
    const oldBuffer = oldView.getBuffer();
    const oldBytesPerEntry = oldView.getBytesPerEntry();

    // Copy entry by entry to handle potential alignment differences
    const oldUint8 = new Uint8Array(oldBuffer);
    const newUint8 = new Uint8Array(newBuffer);
    const entriesToCopy = Math.min(oldView.length, newCapacity);

    for (let i = 0; i < entriesToCopy; i++) {
      const oldOffset = i * oldBytesPerEntry;
      const newOffset = i * alignedBytesPerEntry;
      const bytesToCopy = Math.min(oldBytesPerEntry, alignedBytesPerEntry);
      newUint8.set(
        oldUint8.subarray(oldOffset, oldOffset + bytesToCopy),
        newOffset
      );
    }
