...
export class Pool {
  ...
  get(): number {
    ...
    for (let attempts = 0; attempts < this.bucketCount * 2; attempts++) {
      ...
      for (let i = 0; i < this.bucketCount; i++) {
        const bucketIndex =
          ((hintBucket + i) % this.bucketCount) + Pool.DATA_OFFSET;
    ...
    throw new Error(
      `Entity pool exhausted: maximum of ${this.size} entities reached`
    );
  }
  ...
  private updateHint(emptyBucket: number): void {
    ...
    for (let i = 0; i < this.bucketCount; i++) {
      const bucketIndex =
        ((emptyBucket + i) % this.bucketCount) + Pool.DATA_OFFSET;
      const value = Atomics.load(this.view, bucketIndex);
      if (value !== 0) {
        Atomics.store(
          this.view,
          Pool.HINT_INDEX,
          bucketIndex - Pool.DATA_OFFSET
        );
        return;
      }
    }
