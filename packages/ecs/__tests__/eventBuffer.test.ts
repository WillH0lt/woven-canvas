...
describe("EventBuffer", () => {
  ...
  describe("collectEntitiesInRange", () => {
    ...
    it("should filter by event type", () => {
      ...
      const { entities: addedEntities } = eventBuffer.collectEntitiesInRange(
        0,
        EventType.ADDED
      );
      ...
      const { entities: removedEntities } = eventBuffer.collectEntitiesInRange(
        0,
        EventType.REMOVED
      );
      ...
      const { entities: changedEntities } = eventBuffer.collectEntitiesInRange(
        0,
        EventType.CHANGED
      );
  ...
  describe("ring buffer wrapping", () => {
    it("should wrap around when buffer is full", () => {
      ...
      const { entities } = smallBuffer.collectEntitiesInRange(
        6,
        EventType.ADDED
      );
  ...
  describe("fromTransfer", () => {
    it("should create EventBuffer from shared buffer", () => {
      ...
      const { entities } = transferred.collectEntitiesInRange(
        0,
        EventType.ADDED
      );
      expect(entities.size).toBe(1);
