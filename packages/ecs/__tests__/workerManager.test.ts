...
class MockWorker {
  ...
  static defaultBehavior: "success" | "error" | "error-message" | "manual" =
    "success";
  ...
  postMessage(message: InitMessage | ExecuteMessage): void {
    ...
    if (MockWorker.defaultBehavior === "manual") {
      ...
      MockWorker.manualResolvers.push(() => {
        self.respondSuccess(message.threadIndex);
      });
      return;
    }
    ...
    setTimeout(() => {
      ...
      if (
        ...
        self.triggerError("Test error");
      } else if (
        MockWorker.defaultBehavior === "error-message" &&
        message.type === "execute"
      ) {
        self.respondError(message.threadIndex, "Execution failed");
      } else {
        self.respondSuccess(message.threadIndex);
      }
    }, 0);
  ...
  describe("execute", () => {
    ...
    it("should pass correct threadIndex and threadCount in init message", async () => {
      ...
      MockWorker.prototype.postMessage = function (
        this: MockWorker,
        message: InitMessage | ExecuteMessage
      ) {
