...
import { Intersect } from "../../../src/singletons";
...
import { simulateMouseMove, simulateMouseLeave } from "../../testUtils";
import { PLUGIN_NAME } from "../../../src/constants";

// Factory function to create test plugin with fresh system instance
const createTestPlugin = (domElement: HTMLElement): EditorPlugin => ({
  name: PLUGIN_NAME,
  ...
  inputSystems: [mouseSystem],
  preCaptureSystems: [intersectSystem],
  setup(ctx) {
    // Attach mouse listeners to capture DOM events
...
describe("preCaptureIntersect", () => {
  ...
  beforeEach(async () => {
    ...
    editor = new Editor(domElement, {
      plugins: [createTestPlugin(domElement)],
