...
function getSvgPathFromStroke(
  ...
  for (let i = 0; i < effectiveCount; i++) {
    ...
    inputPoints.push([points[bufferIndex] - left, points[bufferIndex + 1] - top])
  }
...
watch(
  ...
  ([stroke, blockData]) => {
    ...
    path.value = getSvgPathFromStroke(stroke.pointCount, stroke.firstPointIndex, stroke.points, 2 * radius, left, top)
  },
  { immediate: true },
)
