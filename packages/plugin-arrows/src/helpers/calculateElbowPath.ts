...
export function calculateElbowPath(
  ...
  const calculator = new ElbowPathCalculator(ctx, start, end, padding, rotation)
...
class ElbowPathCalculator {
  ...
  constructor(ctx: Context, start: Vec2, end: Vec2, padding: number, rotation: number) {
    this.ctx = ctx
  ...
  calculate(startBlockId: EntityId | null, endBlockId: EntityId | null): Vec2[] {
    const startRay = this.calculateRay(this.localStart, this.localEnd, startBlockId)
    const endRay = this.calculateRay(this.localEnd, this.localStart, endBlockId)
    ...
    if (startBlockId && endBlockId) {
      ...
      const startInEndBlock = this.isPointInsideBlockMargin(this.localStart, endBlockId)
      const endInStartBlock = this.isPointInsideBlockMargin(this.localEnd, startBlockId)
      ...
      if (this.isPointInsideBlockMargin(this.localStart, endBlockId)) {
        ...
        path = this.routeBlockToPoint(endBlockId, endRay, this.localStart).reverse()
      }
    } else {
      path = routePointToPoint(Ray.origin(startRay), this.localEnd)
    }
  ...
  private getBlockAabb(entityId: EntityId): Aabb {
    ...
    const localPosition: Vec2 = [localCenter[0] - block.size[0] / 2, localCenter[1] - block.size[1] / 2]
  ...
  private routeBlockToBlock(startBlockId: EntityId, endBlockId: EntityId, startRay: Ray, endRay: Ray): Vec2[] {
    ...
    if (startId === null || endId === null) {
      console.warn('[routeBlockToBlock] Failed to add start or end node to graph')
  ...
  private routeBlockToPoint(blockId: EntityId, ray: Ray, endPoint: Vec2): Vec2[] {
    ...
    if (rayDir[0] === dominantDirection[0] && rayDir[1] === dominantDirection[1]) {
    ...
    const endRay = Ray.fromPoints(endPoint, [-dominantDirection[0], -dominantDirection[1]])
  ...
  private calculateRay(point: Vec2, target: Vec2, blockId: EntityId | null): Ray {
    let direction: Vec2
...
function removeStraightLinePoints(path: Vec2[]): void {
  ...
  for (let i = 1; i < path.length - 1; i++) {
    ...
    const collinearX = Scalar.approxEqual(prev[0], current[0]) && Scalar.approxEqual(current[0], next[0])
    const collinearY = Scalar.approxEqual(prev[1], current[1]) && Scalar.approxEqual(current[1], next[1])
...
function getNodes(aabb1: Aabb, aabb2: Aabb): Node[] {
  ...
  for (const corner of intersectionCorners) {
    if (!(Aabb.containsPoint(aabb1, corner, false) || Aabb.containsPoint(aabb2, corner, false))) {
      perimeter.push(corner)
    }
  }
...
function intersectRayAndAddNode(ray: Ray, nodes: Node[]): number | null {
  const rayOrigin = Ray.origin(ray)
  const preexistingNode = nodes.find((n) => n.coords[0] === rayOrigin[0] && n.coords[1] === rayOrigin[1])
  if (preexistingNode) {
  ...
  for (const node of nodes) {
    for (const neighborId of node.connections) {
      ...
      const intersection = Ray.intersectSegment(ray, node.coords, neighbor.coords)
