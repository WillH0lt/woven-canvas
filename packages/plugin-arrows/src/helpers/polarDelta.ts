 ...
 */
export function applyPolarDelta(
  position: Vec2,
  angle: number,
  distance: number,
  origin: Vec2
  ...
  const newAngle =
    Math.atan2(position[1] - origin[1], position[0] - origin[0]) + angle;
  const newDistance =
    Math.hypot(position[0] - origin[0], position[1] - origin[1]) + distance;
  return [
    origin[0] + Math.cos(newAngle) * newDistance,
    origin[1] + Math.sin(newAngle) * newDistance,
  ];
}
