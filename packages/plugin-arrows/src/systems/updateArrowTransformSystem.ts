...
import { applyPolarDelta, calculateElbowPath, polarDelta } from '../helpers'
import { ArrowHandleKind, type ArrowHandleKind as ArrowHandleKindType, ArrowHeadKind, ArrowKind } from '../types'
...
function updateArrowForConnectedBlock(ctx: Context, arrowId: EntityId): void {
  ...
  if (hasComponent(ctx, arrowId, ArcArrow)) {
    ...
    if (connector.startBlock !== null) {
      newStart = Block.uvToWorld(ctx, connector.startBlock, connector.startBlockUv)
    ...
    // Update start position if connected
    if (connector.startBlock !== null) {
      const newStart = Block.uvToWorld(ctx, connector.startBlock, connector.startBlockUv)
      updateElbowArrow(ctx, arrowId, ArrowHandleKind.Start, newStart)
    }

    // Update end position if connected
    if (connector.endBlock !== null) {
      const newEnd = Block.uvToWorld(ctx, connector.endBlock, connector.endBlockUv)
 ...
 */
function addArrow(ctx: Context, entityId: EntityId, position: Vec2, kind: ArrowKind): void {
  ...
  const startBlockId = intersects.find((eid) => hasComponent(ctx, eid, Synced) && canConnectToBlock(ctx, eid)) ?? null
  ...
  if (endHandle) {
    ...
    selectionState.draggedEntityStart = [handleBlock.position[0], handleBlock.position[1]]
...
function onArrowHandleDrag(ctx: Context, handleId: EntityId, position: Vec2): void {
  ...
  let handlePosition: Vec2 = [position[0] + handleBlock.size[0] / 2, position[1] + handleBlock.size[1] / 2]
 ...
 */
function addOrUpdateTransformHandles(ctx: Context, arrowEntityId: EntityId): EntityId[] {
  const handleSize = TRANSFORM_HANDLE_SIZE

  // Determine which handle kinds are needed
  const neededKinds: ArrowHandleKindType[] = [ArrowHandleKind.Start, ArrowHandleKind.End]
 ...
 */
function getHandlePosition(ctx: Context, arrowEntityId: EntityId, handleKind: ArrowHandleKindType): Vec2 {
  if (hasComponent(ctx, arrowEntityId, ArcArrow)) {
 ...
 */
function updateArcArrow(ctx: Context, entityId: EntityId, handleKind: ArrowHandleKindType, handlePosition: Vec2): void {
...
function updateElbowArrow(
  ...
  if (handleKind === ArrowHandleKind.Start) {
    ...
    if (connector.startBlock !== null) {
      start = Block.uvToWorld(ctx, connector.startBlock, connector.startBlockUv)
    } else {
...
function updateConnector(
  ...
  if (attachmentBlockId !== null) {
    const otherEnd = handleKind === ArrowHandleKind.Start ? connector.endBlock : connector.startBlock
  ...
  if (attachmentBlockId !== null) {
    ...
    const snapTerminal = findNearestTerminal(ctx, attachmentBlockId, handlePosition, TERMINAL_SNAP_DISTANCE)
