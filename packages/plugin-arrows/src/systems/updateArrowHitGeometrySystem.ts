 ...
 */
export const updateArrowHitGeometrySystem = defineEditorSystem({ phase: 'update', priority: -10 }, (ctx: Context) => {
  // Update arc arrows
...
function updateArcArrowTrim(ctx: Context, entityId: EntityId): void {
  ...
  if (!Scalar.approxEqual(arcArrowRead.trimStart, startTrim) || !Scalar.approxEqual(arcArrowRead.trimEnd, endTrim)) {
 ...
 */
function calculateArcTrim(ctx: Context, blockId: EntityId | null, arc: Arc, referencePoint: Vec2): number {
  if (!blockId) return 0
 ...
 */
function calculateLineTrim(ctx: Context, blockId: EntityId | null, lineStart: Vec2, lineEnd: Vec2): number {
  if (!blockId) return 0
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const intersection = lineSegmentIntersection(lineStart, lineEnd, edgeStart, edgeEnd)
    if (intersection) {
  ...
  const t = ((closest[0] - lineStart[0]) * dx + (closest[1] - lineStart[1]) * dy) / (length * length)
