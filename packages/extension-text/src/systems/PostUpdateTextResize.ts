...
export class PostUpdateTextResize extends BaseSystem {
  ...
  public execute(): void {
    ...
    for (const textEntity of this.selectedTexts.changed) {
      ...
      const isScaling = block.height !== textDragStart.lastHeight || block.width !== textDragStart.lastWidth

      if (isStretching) {
        // if ratio has changed then calculate new block size
        this.stretchBlock(textEntity)
      } else if (isScaling) {
        // scale the font size proportionally
        this.scaleBlock(textEntity)
      }
    }
  ...
  private stretchBlock(textEntity: Entity): void {
    const block = textEntity.read(comps.Block)
  ...
  private scaleBlock(textEntity: Entity): void {
    const block = textEntity.read(comps.Block)
