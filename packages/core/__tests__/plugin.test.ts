import { CanvasComponentDef, CanvasSingletonDef } from '@woven-ecs/canvas-store'
import { field } from '@woven-ecs/core'
import { describe, expect, it, vi } from 'vitest'
import { defineSystem } from '../src'
import { type EditorPlugin, sortPluginsByDependencies } from '../src/plugin'

describe('Plugin System', () => {
  describe('sortPluginsByDependencies', () => {
    it('should return plugins in same order when no dependencies', () => {
      const plugins: EditorPlugin[] = [{ name: 'a' }, { name: 'b' }, { name: 'c' }]
    ...
    it('should sort plugins with single dependency', () => {
      const plugins: EditorPlugin[] = [{ name: 'child', dependencies: ['parent'] }, { name: 'parent' }]
    ...
    it('should throw on missing dependency', () => {
      const plugins: EditorPlugin[] = [{ name: 'child', dependencies: ['nonexistent'] }]
    ...
    it('should throw on circular dependency (direct)', () => {
      ...
      expect(() => sortPluginsByDependencies(plugins)).toThrow('Circular plugin dependency')
    })
  ...
  describe('EditorPlugin interface', () => {
    ...
    it('should support plugin with components', () => {
      ...
      const Selected = new CanvasComponentDef({ name: 'selected', sync: 'ephemeral' }, {})
  ...
  describe('real-world plugin scenarios', () => {
    ...
    it('should correctly order plugin chain', () => {
      ...
      expect(names.indexOf('selection')).toBeLessThan(names.indexOf('transform'))
