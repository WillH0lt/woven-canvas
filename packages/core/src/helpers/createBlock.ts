 ...
 * `position` are required — everything else defaults. `position` is the block's
 * top-left in world coordinates; the block's center (derived from `position` and
 * `size`) is the point used to find the frame to drop the block into. `rank`
 * (z-order) is set to top-of-stack unless you provide one.
 */
export interface CreateBlockInput {
  /** Block type, e.g. `'shape'` or your own `blockDefs` tag. */
  tag: string
  /** Top-left in world coordinates. The block's center (from this + `size`) finds the target frame. */
  position: Vec2
 ...
 * parents the block to the frame under its center and lets other plugins
 ...
 * ```
 *
 * For full control — a placement point different from the block's center, or
