...
export class InputPointer extends BaseSystem {
  private readonly writablePointers = this.query(
    (q) => q.current.with(Pointer).write
  );

  private readonly cameras = this.query(
    (q) => q.changed.with(Camera).trackWrites
  );
  ...
  private onPointerMove(e: PointerEvent, pointers: Entity[]): void {
    const pointerEntity = pointers.find(
      (p) => p.alive && p.read(Pointer).id === e.pointerId
    );
  ...
  public execute(): void {
    ...
    for (const e of this.eventsBuffer) {
      if (e.type === "pointerdown") {
        ...
        const hasPointerDown = !!this.eventsBuffer.find(
          (e) => e.type === "pointerdown"
        );
