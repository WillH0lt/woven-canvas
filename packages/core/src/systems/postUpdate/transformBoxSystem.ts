...
import { ScaleWithZoom } from '../../components/ScaleWithZoom'
...
import { getBlockDef, getBlockResizeMode } from '../../helpers/blockDefs'
import { Camera } from '../../singletons/Camera'
import { SelectionStateSingleton } from '../../singletons/SelectionStateSingleton'
import { ResizeMode, SelectionState, TransformHandleKind } from '../../types'
...
function updateTransformBox(ctx: Context, transformBoxId: EntityId): void {
  ...
  if (!isDragging) {
    ...
    // Store world position so drag handlers can compute world-space geometry
    for (const entityId of selectedBlocks) {
