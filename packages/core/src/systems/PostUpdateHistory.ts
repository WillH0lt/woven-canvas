...
export class PostUpdateHistory extends BaseSystem<CoreCommandArgs> {
  ...
  public constructor() {
    ...
    for (const Comp of Components) {
      ...
      const changedQuery = this.query(
        (q) => q.changed.with(comps.Persistent, Comp).trackWrites
      );
      this.changedQueries.set(Comp, changedQuery);

      const removedQuery = this.query((q) =>
        q.removed.with(comps.Persistent, Comp)
      );
  ...
  public execute(): void {
    ...
    const hasRemoved = Array.from(this.removedQueries.values()).some(
      (q) => q.removed.length > 0
    );
    if (hasRemoved) {
  ...
  private createCheckpoint(): void {
    ...
    this.resources.outwardEmitter.emit("change");
  }
}
