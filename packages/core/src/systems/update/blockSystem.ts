...
function cloneEntities(ctx: Context, entityIds: EntityId[], offset: Vec2, seed: string): void {
  ...
  for (const entityId of entityIds) {
    for (const descendant of getDescendants(ctx, entityId)) {
      allEntityIdsSet.add(descendant)
    }
  }
  ...
  for (const entityId of entityIds) {
    ...
    for (const componentId of ctx.entityBuffer.getComponentIds(entityId)) {
      ...
      // For Block component: only offset root blocks (children keep local positions).
      // Assign pre-computed rank to all blocks.
      if (componentId === blockComponentId) {
        if (rootSet.has(entityId)) {
  ...
  // Remap parentId refs: if a child's parent was cloned, point to the clone
  swapParentRefs(ctx, originalToClone)
}
...
/**
 * Remap parentId references on cloned entities.
 *
 * When both a parent and its child are cloned, the child clone's parentId
 * still points to the original parent. This function updates it to point
 * to the cloned parent instead.
 *
 * @param fromTo - Map of original EntityId → clone EntityId
 */
function swapParentRefs(ctx: Context, fromTo: Map<EntityId, EntityId>): void {
  for (const [, cloneId] of fromTo) {
    if (!isAlive(ctx, cloneId)) continue
    if (!hasComponent(ctx, cloneId, Block)) continue
