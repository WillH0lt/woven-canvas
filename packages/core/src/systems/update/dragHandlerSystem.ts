...
import { DragStart } from '../../components/DragStart'
...
const transformBoxQuery = defineQuery((q) => q.with(Block, TransformBox, DragStart))

/**
 * Check if a block's parent is also selected.
 * Used to skip children during drag — their parent's movement already handles them.
 */
 ...
 */
function onTransformBoxDrag(ctx: Context, boxId: EntityId, position: Vec2): void {
  ...
  for (const blockId of selectedBlocksQuery.current(ctx)) {
    ...
    // Skip children whose parent is also selected — parent movement handles them
 ...
 */
function onTransformHandleDrag(ctx: Context, handleId: EntityId, position: Vec2): void {
 ...
 * @param position - The new position being set on the handle
 */
function onRotateHandleDrag(ctx: Context, handleId: EntityId, position: Vec2): void {
  ...
  const handleStartAngle =
    Vec2.angle([boxBlock.size[0] * handle.vectorX, boxBlock.size[1] * handle.vectorY]) + dragStart.rotateZ
 ...
 */
function onScaleHandleDrag(ctx: Context, handleId: EntityId, position: Vec2, maintainAspectRatio: boolean): void {
  const transformBoxes = transformBoxQuery.current(ctx)
  if (transformBoxes.length === 0) {
  ...
  const oppositeLocal: Vec2 = [(-handle.vectorX * boxStart.size[0]) / 2, (-handle.vectorY * boxStart.size[1]) / 2]
  Vec2.rotate(oppositeLocal, boxRotateZ)
  const oppositeCenter: Vec2 = [boxStartCenter[0] + oppositeLocal[0], boxStartCenter[1] + oppositeLocal[1]]

  // Calculate difference from opposite corner to handle center
  const difference: Vec2 = [handleCenter[0] - oppositeCenter[0], handleCenter[1] - oppositeCenter[1]]
  ...
  for (const blockId of selectedBlocksQuery.current(ctx)) {
    ...
    Vec2.add(blockStartCenter, [blockStart.size[0] / 2, blockStart.size[1] / 2])
    ...
    // Convert center back to top-left position (world space)
    ...
    if (hasComponent(ctx, blockId, Text)) {
      ...
      if (didStretch) {
        ...
        const snappedScaleY = blockStart.size[1] > 0 ? newBlockSize[1] / blockStart.size[1] : 1
