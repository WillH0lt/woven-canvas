...
import { addComponent, type Context, createEntity, defineQuery, removeEntity } from '@woven-ecs/core'
...
import { intersectAabb } from '../../helpers/intersect'
...
export const selectSystem = defineEditorSystem({ phase: 'update' }, (ctx: Context) => {
  ...
  on(ctx, UpdateSelectionBox, (ctx, { bounds, deselectOthers }) => {
    ...
    const intersectedEntityIds = intersectAabb(ctx, bounds, syncedBlocksQuery.current(ctx))
