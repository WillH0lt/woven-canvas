...
function addToolbar(
  ...
  toolbarElement.tools = Object.values(tools).sort(
...
function applyTheme(theme: Theme): void {
  ...
  style.setProperty(
    "--ic-menu-tooltip-border-radius",
    theme.menuTooltipBorderRadius
  );
  style.setProperty("--ic-transition-duration", theme.transitionDuration);
  style.setProperty(
    "--ic-transition-timing-function",
    theme.transitionTimingFunction
  );
  style.setProperty(
    "--ic-highlighted-block-outline-color",
    theme.highlightedBlockOutlineColor
  );
  style.setProperty(
    "--ic-highlighted-block-outline-width",
    theme.highlightedBlockOutlineWidth
  );
  style.setProperty(
    "--ic-highlighted-block-outline-offset",
    theme.highlightedBlockOutlineOffset
  );
  style.setProperty(
    "--ic-highlighted-block-border-radius",
    theme.highlightedBlockBorderRadius
  );
...
export class InfiniteCanvas {
  ...
  public static async New(
    ...
    Camera.persistent = parsedOptions.localPersistence.camera;

    const inwardEmitter = new Emitter<InwardEmitterEvents>();
    const outwardEmitter = new Emitter<EmitterEvents>();
    const state = new State();
    extensions.unshift(new TextEditorExtension(parsedOptions));
    extensions.unshift(
      new CoreExtension(inwardEmitter, outwardEmitter, state, parsedOptions)
    );
    ...
    for (const ext of extensions) {
      for (const floatingMenu of ext.floatingMenus) {
        floatingMenus[floatingMenu.component.name] =
          FloatingMenuDef.parse(floatingMenu);
      }
    }
    ...
    const preInputGroups = extensions
      .map((ext) => ext.preInputGroup)
      .filter((g) => g !== null);
    const inputGroup = extensions
      .map((ext) => ext.inputGroup)
      .filter((g) => g !== null);
    const postInputGroups = extensions
      .map((ext) => ext.postInputGroup)
      .filter((g) => g !== null);

    const preCaptureGroups = extensions
      .map((ext) => ext.preCaptureGroup)
      .filter((g) => g !== null);
    const captureGroups = extensions
      .map((ext) => ext.captureGroup)
      .filter((g) => g !== null);
    const postCaptureGroups = extensions
      .map((ext) => ext.postCaptureGroup)
      .filter((g) => g !== null);

    const preUpdateGroups = extensions
      .map((ext) => ext.preUpdateGroup)
      .filter((g) => g !== null);
    const updateGroups = extensions
      .map((ext) => ext.updateGroup)
      .filter((g) => g !== null);
    const postUpdateGroups = extensions
      .map((ext) => ext.postUpdateGroup)
      .filter((g) => g !== null);

    const preRenderGroups = extensions
      .map((ext) => ext.preRenderGroup)
      .filter((g) => g !== null);
    const renderGroups = extensions
      .map((ext) => ext.renderGroup)
      .filter((g) => g !== null);
    const postRenderGroups = extensions
      .map((ext) => ext.postRenderGroup)
      .filter((g) => g !== null);
    ...
    world.build((worldSys) => {
      ...
      Object.assign(
        worldSys.singleton.write(Background),
  ...
  private constructor(
    ...
    public readonly emitter: Emitter<EmitterEvents>,
    ...
    const send = (kind: string, ...args: any[]) =>
      this.inwardEmitter.emit(InwardEmitterEventKind.Command, {
        kind,
