...
export class DraggableElement {
  ...
  private handleDragStart = (event: Event) => {
    ...
    if (supportsTouch && event.type.startsWith('touch')) {
    ...
    this.element.addEventListener('pointerup', this.handleDragStop)
    this.element.addEventListener('pointermove', this.handleDragMove)
    this.element.addEventListener('touchend', this.handleDragStop)
    this.element.addEventListener('touchmove', this.handleDragMove)
    this.options.start(clientX, clientY)
  }

  private handleDragStop = (event: Event) => {
    let clientX: number
    let clientY: number
  ...
  private handleDragMove = (event: Event) => {
    let clientX: number
    let clientY: number
