...
export class ICBaseBlock extends LitElement {
  ...
  static styles = [
    ...
    */
      ...
      :host([is-selected]) > :first-child::after {
        ...
        width: calc(
          (100% * var(--ic-zoom)) +
            (2 * var(--ic-highlighted-block-outline-offset))
        );
        height: calc(
          (100% * var(--ic-zoom)) +
            (2 * var(--ic-highlighted-block-outline-offset))
        ...
        border: var(--ic-highlighted-block-outline-width) solid
  ...
  protected computeBlockDimensions(element: HTMLElement): {
    width: number;
    height: number;
    left: number;
    top: number;
    ...
    const bottomRight = mCamera.transformPoint(
    ...
    const screen = this.store.core.screen.value;
    const offsetX = screen?.left ?? 0;
    const offsetY = screen?.top ?? 0;
