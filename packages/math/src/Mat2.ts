...
export namespace Mat2 {
  ...
  export const create = (a: number, b: number, c: number, d: number, tx: number, ty: number): Mat2 => [
    a,
    b,
    c,
    d,
  ...
  export const clone = (m: Mat2): Mat2 => [m[A], m[B], m[C], m[D], m[TX], m[TY]]

  /**
   * Create a translation matrix.
   */
  export const fromTranslation = (tx: number, ty: number): Mat2 => [1, 0, 0, 1, tx, ty]

  /**
   * Create a scaling matrix.
   * If sy is not provided, uniform scaling is applied.
   */
  export const fromScale = (sx: number, sy?: number): Mat2 => [sx, 0, 0, sy ?? sx, 0, 0]
   ...
   */
  export const fromSkew = (skewX: number, skewY: number): Mat2 => [1, Math.tan(skewY), Math.tan(skewX), 1, 0, 0]
  ...
  export const set = (out: Mat2, a: number, b: number, c: number, d: number, tx: number, ty: number): void => {
    out[A] = a
    out[B] = b
    out[C] = c
    out[D] = d
   ...
   */
  export const inverseTransformPoint = (m: Mat2, out: Vec2): boolean => {
   ...
   */
  export const toCssMatrix = (m: Mat2): string => `matrix(${m[A]}, ${m[B]}, ${m[C]}, ${m[D]}, ${m[TX]}, ${m[TY]})`
  ...
  export const fromCssMatrix = (css: string): Mat2 | null => {
    const match = css.match(/matrix\(\s*([^,]+),\s*([^,]+),\s*([^,]+),\s*([^,]+),\s*([^,]+),\s*([^)]+)\s*\)/)
    if (!match) {
      return null
    }
