...
export namespace Capsule {
  ...
  export const create = (ax: number, ay: number, bx: number, by: number, radius: number): Capsule => [
    ax,
    ay,
    bx,
    by,
  ...
  export const fromPoints = (a: Vec2, b: Vec2, radius: number): Capsule => [a[0], a[1], b[0], b[1], radius]

  export const clone = (c: Capsule): Capsule => [c[0], c[1], c[2], c[3], c[4]]
  ...
  export const center = (c: Capsule): Vec2 => [(c[CAPSULE_AX] + c[CAPSULE_BX]) / 2, (c[CAPSULE_AY] + c[CAPSULE_BY]) / 2]
   ...
   */
  export const closestPointOnCenterLine = (c: Capsule, point: Vec2): Vec2 => {
    const t = closestPointOnSegmentParam(c[CAPSULE_AX], c[CAPSULE_AY], c[CAPSULE_BX], c[CAPSULE_BY], point[0], point[1])
    return [c[CAPSULE_AX] + t * (c[CAPSULE_BX] - c[CAPSULE_AX]), c[CAPSULE_AY] + t * (c[CAPSULE_BY] - c[CAPSULE_AY])]
  }
  ...
  export const intersectsAabb = (c: Capsule, aabb: Aabb): boolean => {
    ...
    const _aabbLeft = aabb[0]
    const _aabbTop = aabb[1]
    const _aabbRight = aabb[2]
    const _aabbBottom = aabb[3]
   ...
   */
  export const intersectsCapsule = (c1: Capsule, c2: Capsule): boolean => {
  ...
  export const set = (out: Capsule, ax: number, ay: number, bx: number, by: number, radius: number): void => {
    out[CAPSULE_AX] = ax
    out[CAPSULE_AY] = ay
    out[CAPSULE_BX] = bx
    out[CAPSULE_BY] = by
   ...
   */
  export const trim = (out: Capsule, tStart: number, tEnd: number): void => {
