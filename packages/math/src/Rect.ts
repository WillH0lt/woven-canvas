...
export namespace Rect {
   ...
   */
  export const getCorners = (position: Vec2, size: Vec2, rotateZ: number, out: [Vec2, Vec2, Vec2, Vec2]): void => {
   ...
   */
  export const getAabb = (position: Vec2, size: Vec2, rotateZ: number, out: Aabb): void => {
    const halfWidth = size[X] / 2
    const halfHeight = size[Y] / 2
    const cx = position[X] + halfWidth
    const cy = position[Y] + halfHeight
   ...
   */
  export const uvToWorld = (position: Vec2, size: Vec2, rotateZ: number, uv: Vec2): Vec2 => {
   ...
   */
  export const worldToUv = (position: Vec2, size: Vec2, rotateZ: number, world: Vec2): Vec2 => {
   ...
   */
  export const getUvToWorldMatrix = (position: Vec2, size: Vec2, rotateZ: number, out: Mat2): void => {
    const cos = Math.cos(rotateZ)
    const sin = Math.sin(rotateZ)
    const w = size[X]
    const h = size[Y]
   ...
   */
  export const containsPoint = (position: Vec2, size: Vec2, rotateZ: number, point: Vec2): boolean => {
    const halfWidth = size[X] / 2
    const halfHeight = size[Y] / 2
    const cx = position[X] + halfWidth
    const cy = position[Y] + halfHeight
  ...
  export const intersectsAabb = (
    ...
    // Test rectangle axes
    if (isSeparated(tempAabbCorners, tempCorners, tempAxes[0][X], tempAxes[0][Y])) return false
    if (isSeparated(tempAabbCorners, tempCorners, tempAxes[1][X], tempAxes[1][Y])) return false
   ...
   */
  export const scaleBy = (position: Vec2, size: Vec2, scaleFactor: number): void => {
    // Get current center
   ...
   * @param scaleFactor - Scale multiplier
   */
  export const scaleAround = (position: Vec2, size: Vec2, pivot: Vec2, scaleFactor: number): void => {
    // Get current center
    const cx = position[X] + size[X] / 2
    const cy = position[Y] + size[Y] / 2
   ...
   */
  export const rotateAround = (position: Vec2, size: Vec2, rotateZ: number, pivot: Vec2, angle: number): number => {
    // Get current center
    const cx = position[X] + size[X] / 2
    const cy = position[Y] + size[Y] / 2
   ...
   */
  export const boundPoints = (position: Vec2, size: Vec2, rotateZ: number, points: Vec2[]): void => {
