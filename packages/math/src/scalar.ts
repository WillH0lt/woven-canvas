...
export namespace Scalar {
   ...
   */
  export const anglesApproxEqual = (a: number, b: number, epsilon = 0.0001): boolean =>
    Math.abs(normalizeAngleSigned(a - b)) < epsilon

  /**
   * Check if two numbers are approximately equal (within epsilon).
   */
  export const approxEqual = (a: number, b: number, epsilon = 0.0001): boolean => Math.abs(a - b) < epsilon
}
