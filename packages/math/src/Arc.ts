 ...
 */
export type Arc = [aX: number, aY: number, bX: number, bY: number, cX: number, cY: number, thickness: number]
...
export namespace Arc {
  ...
  export const clone = (arc: Arc): Arc => [arc[0], arc[1], arc[2], arc[3], arc[4], arc[5], arc[6]]
  ...
  export const isCollinear = (arc: Arc): boolean => {
    const det = (arc[AX] - arc[CX]) * (arc[BY] - arc[CY]) - (arc[BX] - arc[CX]) * (arc[AY] - arc[CY])
   ...
   */
  export const inArcAngle = (angle: number, startAngle: number, endAngle: number, clockwise: boolean): boolean => {
   ...
   */
  export const intersectsCapsule = (arc: Arc, capsuleA: Vec2, capsuleB: Vec2, _capsuleRadius: number): boolean => {
    const computed = compute(arc)
    if (!computed) return false
    ...
    for (const testRadius of [outerRadius, innerRadius]) {
      ...
      if (aInCircle !== bInCircle) {
        const angle = Math.atan2(capsuleA[1] - center[1], capsuleA[0] - center[0])
        if (inArcAngle(angle, startAngle, endAngle, clockwise)) {
          return true
        }
      }
   ...
   */
  export const intersectSegment = (arc: Arc, p1: Vec2, p2: Vec2): Vec2[] => {
    const computed = compute(arc)
    if (!computed) return []
  ...
  export const pointToParametric = (arc: Arc, point: Vec2): number | null => {
    ...
    if (!inArcAngle(angle, computed.startAngle, computed.endAngle, clockwise)) {
      return null
    }
  ...
  export const parametricToPoint = (arc: Arc, t: number): Vec2 => {
    ...
    if (!computed) {
      ...
      return [arc[AX] + t * (arc[CX] - arc[AX]), arc[AY] + t * (arc[CY] - arc[AY])]
    }
   ...
   */
  export const setFromPoints = (out: Arc, a: Vec2, b: Vec2, c: Vec2, thickness: number): void => {
    out[AX] = a[0]
    out[AY] = a[1]
    out[BX] = b[0]
    out[BY] = b[1]
...
function circleContainsPoint(center: Vec2, radius: number, point: Vec2): boolean {
  const dx = point[0] - center[0]
  const dy = point[1] - center[1]
...
function segmentCircleIntersections(p1: Vec2, p2: Vec2, center: Vec2, r: number): Vec2[] {
