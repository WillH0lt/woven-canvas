...
export namespace Vec2 {
  ...
  export const clone = (v: Vec2): Vec2 => [v[0], v[1]]
   ...
   */
  export const angleTo = (from: Vec2, to: Vec2): number => Math.atan2(to[1] - from[1], to[0] - from[0])

  /**
   * Get the angle of a vector from the origin in radians.
   * Returns angle in range [-π, π].
   */
  export const angle = (v: Vec2): number => Math.atan2(v[1], v[0])

  /**
   * Create a Vec2 from polar coordinates (radius and angle).
   * Optionally offset by a center point.
   */
  export const fromPolar = (radius: number, angle: number, center: Vec2 = [0, 0]): Vec2 => [
  ...
  export const set = (out: Vec2, x: number, y: number): void => {
    out[0] = x
    out[1] = y
  }

  export const copy = (out: Vec2, v: Vec2): void => {
    out[0] = v[0]
    out[1] = v[1]
  }

  export const add = (out: Vec2, v: Vec2): void => {
    out[0] += v[0]
    out[1] += v[1]
  }
   ...
   */
  export const addScalar = (out: Vec2, s: number): void => {
    out[0] += s
    out[1] += s
  }

  export const sub = (out: Vec2, v: Vec2): void => {
    out[0] -= v[0]
    out[1] -= v[1]
  }

  export const scale = (out: Vec2, s: number): void => {
    out[0] *= s
    out[1] *= s
  }
   ...
   */
  export const multiply = (out: Vec2, v: Vec2): void => {
    out[0] *= v[0]
    out[1] *= v[1]
  }
   ...
   */
  export const divide = (out: Vec2, v: Vec2): void => {
    if (v[0] === 0 || v[1] === 0) {
  ...
  export const negate = (out: Vec2): void => {
    out[0] = -out[0]
    out[1] = -out[1]
  }

  export const normalize = (out: Vec2): void => {
   ...
   */
  export const rotate = (out: Vec2, angle: number): void => {
   ...
   */
  export const rotateAround = (out: Vec2, pivot: Vec2, angle: number): void => {
    sub(out, pivot)
    rotate(out, angle)
    add(out, pivot)
  }
