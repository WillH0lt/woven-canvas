...
export namespace Aabb {
  ...
  export const create = (left: number, top: number, right: number, bottom: number): Aabb => [left, top, right, bottom]
  ...
  export const fromPositionSize = (left: number, top: number, width: number, height: number): Aabb => [
    left,
    top,
    left + width,
    top + height,
  ...
  export const center = (a: Aabb): Vec2 => [(a[AABB_LEFT] + a[AABB_RIGHT]) / 2, (a[AABB_TOP] + a[AABB_BOTTOM]) / 2]
  ...
  export const containsPoint = (a: Aabb, point: Vec2, inclusive = true): boolean =>
    inclusive
      ? point[0] >= a[AABB_LEFT] && point[0] <= a[AABB_RIGHT] && point[1] >= a[AABB_TOP] && point[1] <= a[AABB_BOTTOM]
      : point[0] > a[AABB_LEFT] && point[0] < a[AABB_RIGHT] && point[1] > a[AABB_TOP] && point[1] < a[AABB_BOTTOM]
  ...
  export const set = (out: Aabb, left: number, top: number, right: number, bottom: number): void => {
    out[AABB_LEFT] = left
    out[AABB_TOP] = top
