...
const hoveredHandleQuery = defineQuery((q) => q.with(Hovered, TransformHandle).tracking(Hovered))

// Query for transform box - track Block changes for rotation updates
const transformBoxQuery = defineQuery((q) => q.with(Block, TransformBox).tracking(Block))
