...
<Code code={EraserExampleRaw} lang="vue" title="EraserExample.vue" />

Select the Eraser tool and draw over objects to delete them. Objects turn red when marked for deletion. Release to confirm or press Escape to cancel.

## Installation

```bash
npm install @woven-canvas/plugin-eraser
```
...
import { EraserTool, SelectTool, Toolbar } from '@woven-canvas/vue'
</script>
