...
function floor(value: number, step: number): number {
...
export class UpdateEnforceGrid extends BaseSystem {
  private readonly shapes = this.query((q) => q.changed.with(Persistent, Block).trackWrites.write.withAny(Shape))
  ...
  public execute(): void {
    ...
    for (const shapeEntity of this.shapes.changed) {
      ...
      const newLeft = floor(block.left, this.grid.colWidth)
      const newTop = floor(block.top, this.grid.rowHeight)
      const newWidth = ceil(block.width, this.grid.colWidth)
      const newHeight = ceil(block.height, this.grid.rowHeight)
