<template>
  ...
  >
    ...
    >
      ...
      <ElementButton class="relative w-[320px] bg-white drop-shadow" @click="signinGoogleRedirect">
        <svgoGoogle class="absolute left-0 mx-[16px] w-[16px]" />
      ...
      <ElementButton class="relative w-[320px] bg-white drop-shadow" @click="signinGithubRedirect">
        <SvgoGithub class="absolute left-0 mx-[16px] w-[16px]" />
        <div class="text-black">Continue with Github</div>
      </ElementButton>

      <div v-if="errorMessage" class="w-[320px] text-error">* {{ errorMessage }}</div>

      <div class="w-[320px] text-gray-500 text-center" v-if="signingUp">
        By registering you agree to ScrollyPage's
        <a class="text-primary hover:underline" href="https://scrolly.page/terms" target="_blank"
          >Terms of Service</a
        >
        and
        <a class="text-primary hover:underline" href="https://scrolly.page/privacy" target="_blank"
          >Privacy Policy</a
        >
        .
      </div>
...
const AuthErrorText = {
  ...
  'auth/email-already-in-use': 'the email address is already being used by another account.',
  ...
  'auth/provider-already-linked': 'The account is already linked to an existing account.',
  'auth/invalid-phone-number': 'Please enter a valid phone number.',
  'auth/requires-recent-login': 'This requires a recent log in, please log in again and retry.',
...
async function signinGoogleRedirect() {
  ...
  try {
    ...
    console.error(err);
    errorMessage.value =
      err.code in AuthErrorText ? AuthErrorText[err.code as keyof typeof AuthErrorText] : err;
  }
}
