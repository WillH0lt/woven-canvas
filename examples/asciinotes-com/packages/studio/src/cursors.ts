import { CursorKind, PointerAction } from './types.js';

const AUTO_CURSOR_SVG = `<?xml version="1.0" encoding="UTF-8"?><svg id="Layer_1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 4.35 4.75" width="20" height="20"><defs><style>.cls-1{stroke:#fff;stroke-miterlimit:10;stroke-width:.25px;}</style></defs><path class="cls-1" d="M2.39,2.88s-.04.01-.05.02c-.01,0-.02.01-.03.02,0,.01-.02.02-.03.05l-.77,1.44c-.07.13-.1.19-.14.2-.03.01-.07,0-.1-.01-.03-.02-.05-.09-.09-.23L.16.41c-.03-.13-.05-.19-.03-.23.02-.03.04-.05.08-.06.04,0,.1.02.21.08l3.59,1.94c.13.07.19.1.2.14.01.03,0,.07-.01.1-.02.03-.09.05-.23.09l-1.58.4Z"/></svg>`;
...
export function setCursor(
  ...
  switch (cursorKind) {
    case CursorKind.Auto:
      if (actionLeftMouse === PointerAction.Pan) {
        document.body.style.cursor = `url("data:image/svg+xml,${encodeURIComponent(HAND_CURSOR_SVG)}") 12 12, auto`;
      } else {
        document.body.style.cursor = `url("data:image/svg+xml,${encodeURIComponent(AUTO_CURSOR_SVG)}") 2 1, auto`;
      }
      break;
    case CursorKind.Move:
      document.body.style.cursor = `url("data:image/svg+xml,${encodeURIComponent(MOVE_CURSOR_SVG)}") 12 12, auto`;
      break;
    case CursorKind.EW:
      document.body.style.cursor = `url("data:image/svg+xml,${encodeURIComponent(NS_CURSOR_SVG(rotateZ + Math.PI / 2))}") 12 12, auto`;
      break;
    case CursorKind.NS:
      document.body.style.cursor = `url("data:image/svg+xml,${encodeURIComponent(NS_CURSOR_SVG(rotateZ))}") 12 12, auto`;
      break;
    case CursorKind.NWSE:
      document.body.style.cursor = `url("data:image/svg+xml,${encodeURIComponent(NS_CURSOR_SVG(rotateZ - Math.PI / 4))}") 12 12, auto`;
      break;
    case CursorKind.NESW:
      document.body.style.cursor = `url("data:image/svg+xml,${encodeURIComponent(NS_CURSOR_SVG(rotateZ + Math.PI / 4))}") 12 12, auto`;
      break;
    case CursorKind.NW:
      document.body.style.cursor = `url("data:image/svg+xml,${encodeURIComponent(NW_CURSOR_SVG(rotateZ))}") 16 16, auto`;
      break;
    case CursorKind.NE:
      document.body.style.cursor = `url("data:image/svg+xml,${encodeURIComponent(NW_CURSOR_SVG(rotateZ + Math.PI / 2))}") 16 16, auto`;
      break;
    case CursorKind.SW:
      document.body.style.cursor = `url("data:image/svg+xml,${encodeURIComponent(NW_CURSOR_SVG(rotateZ - Math.PI / 2))}") 16 16, auto`;
      break;
    case CursorKind.SE:
      document.body.style.cursor = `url("data:image/svg+xml,${encodeURIComponent(NW_CURSOR_SVG(rotateZ - Math.PI))}") 16 16, auto`;
      break;
    case CursorKind.Crosshair:
      document.body.style.cursor = `url("data:image/svg+xml,${encodeURIComponent(CROSSHAIR_CURSOR_SVG)}") 12 12, auto`;
      break;
    default:
      break;
  }
