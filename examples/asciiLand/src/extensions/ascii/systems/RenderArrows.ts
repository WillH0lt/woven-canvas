...
export class RenderArrows extends BaseSystem<ArrowCommandArgs> {
  ...
  private drawLines(arrowEntity: Entity, material: LetterMaterial): void {
    ...
    for (let i = 1; i < elbowArrow.pointCount; i++) {
      ...
      if (i === 1) {
        ...
        start = this.trimStart(start, end, arrowTrim.tStart)
      }
      if (i === elbowArrow.pointCount - 1) {
        const arrowTrim = arrowEntity.read(ArrowTrim)
        end = this.trimStart(end, start, 1 - arrowTrim.tEnd)

        // console.log('trimmed end', arrowTrim.tEnd)
      }
  ...
  private trimStart(start: [number, number], end: [number, number], trim: number): [number, number] {
